...
function Country({ match }) {
    ...
    return (<div className='country'>
        ...
        <ImageEngLeague heading1={state ? state[0].strLeague : " "} heading2={state ? state[0].strSport : " "}></ImageEngLeague>
        {
            state ? state.map(item => {
                return (
                    <CustomLink key={item.idTeam} mainText={item.strTeam} linkTo={`/team/${item.idTeam}`} secondText={item.strStadiumLocation}></CustomLink>
                )
            }) : " "
        }
        <CustomLink mainText={state ? state[0].strTeam : " "} secondText={state ? state[0].strStadiumLocation : " "}></CustomLink>
